 ...
 *   getPolynom()      => null
 */
function getPolynom(...allCoefficients) {
  return function polynom(x) {
    if (allCoefficients.length === 0) {
      return null;
    }
    let result = 0;
    for (let i = 0; i < allCoefficients.length; i += 1) {
      result += allCoefficients[i] * x ** (allCoefficients.length - 1 - i);
    }
    return result;
  };
}
...
function logger(func, logFunc) {
  return function newLogger(...args) {
    const arrayString = [];
    for (let i = 0; i < args.length; i += 1) {
      arrayString.push(JSON.stringify(args[i]));
    }
    const allArgsString = arrayString.join(',');
    logFunc(`${func.name}(${allArgsString}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${allArgsString}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function result(...secondArgs) {
    return fn(...args1, ...secondArgs);
  };
}
